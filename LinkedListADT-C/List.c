...
bool equals(List A, List B){
    ...
    if(A == NULL ){
        fprintf(stderr, "List A Error: calling equals() on List A, which is NULL\n");
        exit(EXIT_FAILURE);
    }
    if(B == NULL ){
        fprintf(stderr, "List B Error: calling equals() on List B, which is NULL.\n");
        exit(EXIT_FAILURE);
    }
...
List copyList(List L){
    ...
    if(L != NULL){
        Node curr = L->head;
        int i = 0;
        while(i < L->num_elements){
            append(listCopy, curr->val);
            curr=curr->next;
            i++;
        }
    }
